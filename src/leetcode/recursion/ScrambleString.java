...
public class ScrambleString {
	public class Solution {
	     public boolean isScramble(String s1, String s2) {
	        ...
	        if(!new_s1.equals(new_s2)) return false;
	        for(int i=1;i<s1.length();i++){
	            ...
	            if(isScramble(sub_s1,sub_s2)&&isScramble(sub_s1_res,sub_s2_res)) return true;
	            sub_s2=s2.substring(s1.length()-i);
	            sub_s2_res=s2.substring(0,s1.length()-i);
	            if(isScramble(sub_s1,sub_s2)&&isScramble(sub_s1_res,sub_s2_res)) return true;
	        }
	        return false;
	}
